...
class QueryExtractionService:
    ...
    def extract_query(self, diagram: str) -> str:
        try:
            ...
            logger.exception(f"Failed to extract query: {e}")
            raise
    ...
    def find_extractor(self, diagram: str) -> QueryExtractorPort:
        try:
            ...
            logger.exception(f"Failed to find extractor: {e}")
            raise
    
    # Optimize extracted keywords by weighting and filtering structural terms
    def optimize_keywords(self, keywords: List[str]) -> Tuple[List[str], List[str]]:
        try:
            ...
            for keyword in keywords:
                # Exclude structural terms from the rag similiarity search query
                if keyword in structural_terms:
                    excluded_terms.append(keyword)
                    continue
                
                # Weight technical IDs lower, business terms higher
            ...
            logger.exception(f"Failed to optimize keywords: {e}")
            raise
    ...
    def log_extraction_results(self, diagram_type: str, keywords: List[str], excluded_terms: List[str]) -> None:
        try:
            ...
            logger.exception(f"Failed to log extraction results: {e}")
            raise
